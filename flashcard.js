...
const App = (() => {
  ...
  async function fetchJson(path) {
    ...
    /* WebView XHR polyfill'da status 0 ham muvaffaqiyat */
    if (!res.ok && res.status !== 0) throw new Error('Fetch failed: ' + path);
    return res.json();
  }
  ...
  function hidePageLoader() {
    ...
    /* Splash ekranini 2.5 soniyadan keyin yashiramiz */
    const splash = document.querySelector('[data-splash-screen]');
    if (splash) {
      setTimeout(() => {
        splash.classList.add('hiding');
        setTimeout(() => { if (splash.parentNode) splash.remove(); }, 700);
      }, window.IS_ANDROID_WEBVIEW ? 2500 : 1800);
    }
  }

  /** Service Worker'ni ro'yxatdan o'tkazadi (PWA offline rejimi) */
  function registerServiceWorker() {
    /* WebView'da Service Worker ishlamaydi — o'tkazib yuboramiz */
    if (!window.IS_ANDROID_WEBVIEW && 'serviceWorker' in navigator) {
    ...
    /* WebView'da navigator.onLine har doim true bo'lishi mumkin, xavfsiz tekshirish */
    try { if (!navigator.onLine) updateOfflineBanner(true); } catch(e) {}
  }
    ...
    main.innerHTML = `
        ...
        <div class="hero-banner-content">
          <div class="hero-logo-mark">
            <svg viewBox="0 0 64 64" width="64" height="64" fill="none">
              <path d="M32 16C26 11 16 11 10 13v34c6-2 16-2 22 3V16z" fill="#2563EB"/>
              <path d="M32 16c6-5 16-5 22-3v34c-6-2-16-2-22 3V16z" fill="#fff"/>
              <path d="M32 22c4-3 10-3 14-2v24c-4-1-10-1-14 2V22z" fill="#E11D48"/>
            </svg>
          </div>
          <h1 class="hero-app-title">Czech<span>Master</span></h1>
          <p class="hero-app-subtitle">Chex tilini oson va samarali o'rganing</p>
        </div>
      </div>

      <a href="#lessons" class="hero-cta-btn">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor"><path d="M10 1L2 5l8 4 8-4-8-4zM2 9l8 4 8-4M2 13l8 4 8-4"/></svg>
        O'qishni boshlash
      </a>

      <div class="home-grid stagger">
        <a href="#lessons" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#3B82F6,#2563EB)">
            <svg viewBox="0 0 24 24" fill="white" width="26" height="26"><path d="M12 5.5A6.5 6.5 0 003 4v13a5.5 5.5 0 016 1V5.5zM21 4a6.5 6.5 0 00-9 1.5V18a5.5 5.5 0 016-1V4z"/></svg>
          </div>
          <span class="home-tile-label">Darslar</span>
        </a>
        <a href="#vocabulary" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#A78BFA,#7C3AED)">
            <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2zm2 5v2h4V8H7zm0 4v2h10v-2H7zm0 4v2h10v-2H7zm8-9l2 3h-4l2-3z"/></svg>
          </div>
          <span class="home-tile-label">So'zlar</span>
        </a>
        <a href="#listening" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#FBBF24,#F59E0B)">
            <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M12 3a7 7 0 00-7 7v4a3 3 0 003 3h1v-7H7v-0a5 5 0 0110 0v0h-2v7h1a3 3 0 003-3v-4a7 7 0 00-7-7z"/></svg>
          </div>
          <span class="home-tile-label">Tinglash</span>
        </a>
        <a href="#speaking" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#34D399,#10B981)">
            <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M20 2H4a2 2 0 00-2 2v18l4-4h14a2 2 0 002-2V4a2 2 0 00-2-2zM7 9h10v2H7V9zm0-3h10v2H7V6z"/></svg>
          </div>
          <span class="home-tile-label">Mashqlar</span>
        </a>
        <a href="#progress" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#F87171,#EF4444)">
            <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M4 13h4v8H4v-8zm6-9h4v17h-4V4zm6 5h4v12h-4V9z"/></svg>
          </div>
          <span class="home-tile-label">Progress</span>
        </a>
        <a href="#settings" class="home-tile">
          <div class="home-tile-icon" style="background:linear-gradient(135deg,#94A3B8,#64748B)">
            <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M12 8a4 4 0 100 8 4 4 0 000-8zm9 4a7 7 0 01-.1 1.2l2.1 1.6-2 3.5-2.5-1a7 7 0 01-2 1.2l-.4 2.7h-4l-.4-2.7a7 7 0 01-2-1.2l-2.5 1-2-3.5 2.1-1.6A7 7 0 013 12a7 7 0 01.1-1.2L1 9.2l2-3.5 2.5 1a7 7 0 012-1.2L7.9 2.8h4l.4 2.7a7 7 0 012 1.2l2.5-1 2 3.5-2.1 1.6A7 7 0 0121 12z"/></svg>
          </div>
          <span class="home-tile-label">Sozlamalar</span>
        </a>
      </div>
      ...
      <div class="streak-card">
        ...
        <div class="streak-card-info">
          <div class="streak-card-count">${stats.streak}</div>
          <div class="streak-card-label">Kunlik seriya</div>
        </div>
        <div class="streak-card-progress">
          <div class="streak-card-bar">
            <div class="streak-card-fill" style="width:${Math.min(100, Math.round((stats.streak / 30) * 100))}%"></div>
          </div>
          <div class="streak-card-target">${stats.streak} / 30 kun</div>
        </div>
      </div>
      ...
      <div class="section">
        <div class="section-header">
          <h2 class="section-title">🇨🇿 Chexiya — Praga</h2>
        </div>
        <div class="prague-gallery">
          <figure class="prague-photo prague-photo-wide">
            <img src="images/prague-bridge.jpg" alt="Charlz ko'prigi" loading="lazy">
            <figcaption>Charlz ko'prigi — Karlův most</figcaption>
          </figure>
          <figure class="prague-photo">
            <img src="images/prague-castle.jpg" alt="Praga qal'asi" loading="lazy">
            <figcaption>Praga qal'asi — Pražský hrad</figcaption>
          </figure>
          <figure class="prague-photo">
            <img src="images/prague-river.jpg" alt="Vltava daryosi" loading="lazy">
            <figcaption>Vltava daryosi</figcaption>
          </figure>
          <figure class="prague-photo">
            <img src="images/prague-rooftops.jpg" alt="Eski shahar" loading="lazy">
            <figcaption>Eski shahar — Staré Město</figcaption>
          </figure>
          <figure class="prague-photo">
            <img src="images/prague-tower.jpg" alt="Ko'prik minorasi" loading="lazy">
            <figcaption>Ko'prik minorasi</figcaption>
          </figure>
        </div>
      </div>
    `;
